...
with open('src/pages/LMSTugas.tsx', 'r') as f:
    code = f.read()

# code now is just base_code. Let's make sure it doesn't have closing tags at the very end.
code = re.sub(r'    </div>\s*\)\s*;\s*\}\s*$', '', code)
code = re.sub(r'    </div>\s*\}\s*\)\s*;\s*\}\s*$', '', code)
# wait, the original closing tag was `    </div>\n  );\n}`
code = re.sub(r'    </div>\s*\n\s*\);\s*\n\}\s*$', '', code)
...
with open('src/pages/LMSTugas.tsx', 'w') as f:
    f.write(code + "\n" + to_add)
